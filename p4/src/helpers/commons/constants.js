...
export const getDefaultMessageData = () => {
  let allMessages = createMessage([], 21, 21, 'Miss Boss Lady', 'Welcome to Alpacan Mingle! The best place to find an lover and for beautiful Alpacas to find the beautiful and wonderful... You!! Exited to have you here.')
  allMessages = createMessage(allMessages, 21, 21, 'Miss Boss Lady', 'You can start by taking the Llama or Alpaca test. Link is on your home page! Only after you take the Llama or Alpaca test, you can chat with other Alpacas!')
  allMessages = createMessage(allMessages, 21, 21, 'Miss Boss Lady', 'If you need a guide on differentiating between Llamas and Alpacas, check out https://www.alpacadirect.com/blogs/alpaca-direct-blog/alpaca-vs-llama-7-ways-to-tell-the-difference')
  return allMessages
}
...
export const createMessage = (allMessages, userIdFrom, userId, usernameTo, message) => {
  ...
  const conversationWithUserIndex = updatedAllMessages.findIndex(convo => convo.userId === userId)
  if (~conversationWithUserIndex) { // have talked to this user before
    const updatedConversation = addMessageToConversation(updatedAllMessages[conversationWithUserIndex], userIdFrom, message)
